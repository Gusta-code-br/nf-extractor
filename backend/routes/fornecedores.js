...
// GET /api/fornecedores?busca=xx&ativo=true
router.get('/', async (req, res) => {
  ...
  const { rows } = await db.query(
    `SELECT * FROM fornecedores ${where} ORDER BY razao_social ASC`, params
  );
  res.json(rows);
});
...
// GET /api/fornecedores/buscar/cnpj?valor=xx
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      `UPDATE fornecedores SET razao_social=$1, cnpj=$2, fantasia=$3, atualizado_em=NOW()
...
router.patch('/:id/status', async (req, res) => {
  ...
  const { rows } = await db.query(
    `UPDATE fornecedores SET ativo=$1, atualizado_em=NOW() WHERE id=$2 RETURNING *`,
    [ativo, req.params.id]
  );
  if (!rows.length) return res.status(404).json({ error: 'Não encontrado' });
  res.json(rows[0]);
